...
import com.example.navigation.Routes.AWARD_SIGHTS_MODE
...
sealed class MapMode {
    ...
    companion object {
        fun fromString(value: String?): MapMode {
            return when (value) {
                SELECTED_SIGHT_MODE -> Chosen
                ALL_SIGHTS_MODE -> All
                AWARD_SIGHTS_MODE -> Award
                else -> throw IllegalArgumentException(
                    "Invalid mode: $value. Expected one of: $SELECTED_SIGHT_MODE, $ALL_SIGHTS_MODE, $AWARD_SIGHTS_MODE"
                )
            }
        }
    }
