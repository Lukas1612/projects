...
fun GrantPermissions(
    ...
    val baseForegroundPermissions = listOf(
        Manifest.permission.ACCESS_FINE_LOCATION,
        Manifest.permission.ACCESS_COARSE_LOCATION
    )

    val additionalPermissionsForApi33Plus = listOf(
        Manifest.permission.POST_NOTIFICATIONS
    )

    val foregroundPermissions =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            baseForegroundPermissions + additionalPermissionsForApi33Plus
        } else {
            baseForegroundPermissions
        }
    ...
    // Permission UI fades in/out as needed
    AnimatedVisibility(
        visible = !allForegroundGranted || (backgroundRequired && !backgroundGranted),
...
private fun PermissionsUI(
    ...
    Column(
        ...
        AnimatedVisibility(
            visible = backgroundRequired && !backgroundGranted,
            enter = fadeIn() + slideInVertically(initialOffsetY = { it / 2 }),
            exit = fadeOut() + slideOutVertically(targetOffsetY = { -it / 2 })
        ) {
...
private fun ForegroundPermissionButton(
    ...
    val buttonText = "Grant Location and Notification Permissions"
